...
use File::Slurp; # Importing module to read file contents
...
sub cmd_operwall {
    ...
    if ($args =~ m/^(status|help|example|sendfile)/i) {
...
# New subroutine to handle sending ASCII art from a file
sub cmd_operwall_sendfile {
    my ($args, $server, $item) = @_;
    my ($filepath) = $args =~ /(\S+)/;

    if (!$filepath) {
        ho_print_error("Usage: /operwall sendfile <filepath>");
        return;
    }

    if (!-e $filepath) {
        ho_print_error("File not found: $filepath");
        return;
    }

    my @lines = read_file($filepath, chomp => 1);
    send_operwall($_, $server, $item) for @lines;
}
...
Irssi::command_bind('operwall sendfile', 'cmd_operwall_sendfile'); # Binding the new command
